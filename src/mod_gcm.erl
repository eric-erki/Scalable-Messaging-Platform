...
-export([start/2, stop/1, push_notification/8,
	 push_notification_with_custom_fields/9,
	 enable_offline_notification/5, disable_notification/3,
	 receive_offline_packet/3,
	 %% other clients may be online, but we still want to push to this one
	 send_offline_packet_notification/5, resend_badge/1,
	 multi_resend_badge/1, offline_resend_badge/0,
	 device_reset_badge/5, apply_on_all_ios_for/3,
	 remove_user/2, process_sm_iq/3]).
...
start(Host, Opts) ->
    case init_host(Host) of
      true ->
	  mnesia:create_table(gcm_cache,
			      [{disc_copies, [node()]},
			       {attributes, record_info(fields, gcm_cache)},
			       {type, bag}]),
	  mnesia:add_table_index(gcm_cache, device_id),
	  mnesia:add_table_copy(gcm_cache, node(), ram_copies),
	  ejabberd_hooks:add(p1_push_notification, Host, ?MODULE,
	  ...
	  ejabberd_hooks:add(p1_push_disable, Host, ?MODULE,
			     disable_notification, 50),
	  ejabberd_hooks:add(remove_user, Host, ?MODULE,
			     remove_user, 50),
	  ejabberd_hooks:add(offline_message_hook, Host, ?MODULE,
			     receive_offline_packet, 35),
	  IQDisc = gen_mod:get_opt(
                       iqdisc, Opts, fun gen_iq_handler:check_type/1,
                       one_queue),
	  gen_iq_handler:add_iq_handler(ejabberd_sm, Host,
					?NS_P1_PUSH, ?MODULE, process_sm_iq,
					IQDisc);
      false ->
	  ?ERROR_MSG("Cannot start ~s on host ~s. "
		     "Check you had the correct license "
		     "for the domain and # of registered users",
		     [?MODULE, Host])
    end.
...
stop(Host) ->
    ...
    ejabberd_hooks:delete(p1_push_enable_offline, Host,
			  ?MODULE, enable_offline_notification, 50),
    ejabberd_hooks:delete(p1_push_disable, Host, ?MODULE,
			  disable_notification, 50),
...
push_notification(Host, JID, Notification, Msg, Unread,
		  Sound, AppID, Sender) ->
    push_notification_with_custom_fields(Host, JID,
					 Notification, Msg, Unread, Sound,
					 AppID, Sender, []).

push_notification_with_custom_fields(Host, JID,
				     Notification, Msg, Unread, Sound, AppID,
				     Sender, CustomFields) ->
    Type = xml:get_path_s(Notification,
			  [{elem, <<"type">>}, cdata]),
    case Type of
      <<"gcm">> ->
	  DeviceID = xml:get_path_s(Notification,
				    [{elem, <<"id">>}, cdata]),
	  PushPacket = build_push_packet(DeviceID, Msg, Unread,
					 Sound, Sender, JID, CustomFields),
	  route_push_notification(Host, JID, AppID, PushPacket),
	  stop;
      _ -> ok
    end.

build_push_packet(DeviceID, Msg, Unread, Sound, Sender,
		  JID, CustomFields) ->
    Badge = jlib:integer_to_binary(Unread),
    SSound = if Sound -> <<"true">>;
		true -> <<"false">>
	     end,
...
enable_offline_notification(JID, Notification, SendBody,
			    SendFrom, AppID1) ->
    Type = xml:get_path_s(Notification,
			  [{elem, <<"type">>}, cdata]),
    case Type of
	  ...
	  case DeviceID of
		...
		AppID = case xml:get_path_s(Notification,
					    [{elem, <<"appid">>}, cdata])
			    of
			  <<"">> -> AppID1;
			  A -> A
		...
		store_cache(JID, ID1, AppID, SendBody, SendFrom,
			    TimeStamp);
	    _ -> ok
	  end,
	  stop;
...
disable_notification(JID, Notification, _AppID) ->
    ...
    case Type of
	  ...
	  case DeviceID of
	    ID1 when is_binary(ID1), size(ID1) > 0 ->
		delete_cache(JID, ID1), stop;
	    _ -> ok
	  end;
      _ -> ok
    end.

do_send_offline_packet_notification(From, To, Packet,
    ...
    if Body == <<"">>; Pushed -> ok;
	   ...
	   IncludeBody = case SendBody of
			   all -> true;
			   first_per_user -> BadgeCount == 1;
			   first -> BadgeCount == 1;
			   none -> false
			 end,
	   Msg = if IncludeBody ->
			...
			case SendFrom of
			  jid -> prepend_sender(SFrom, CBody);
			  username ->
			      UnescapedFrom = unescape(BFrom#jid.user),
			      prepend_sender(UnescapedFrom, CBody);
			  name ->
			      Name = get_roster_name(To, BFrom),
			      prepend_sender(Name, CBody);
			  _ -> CBody
			end;
		    true -> <<"">>
		 end,
	   SSound = if IncludeBody -> <<"true">>;
		       true -> <<"false">>
		    end,
	   Badge = jlib:integer_to_binary(BadgeCount),
	   STo = jlib:jid_to_string(To),
	   Packet1 = #xmlel{name = <<"message">>, attrs = [],
			    children =
				[#xmlel{name = <<"push">>,
					attrs = [{<<"xmlns">>, ?NS_P1_PUSH}],
					children =
...
send_offline_packet_notification(From, To, Packet,
				 DeviceID, BadgeCount) ->
    case DeviceID of
      ID1 when is_binary(ID1), size(ID1) > 0 ->
...
receive_offline_packet(From, To, Packet) ->
    ?DEBUG("mod_gcm offline~n\tfrom ~p~n\tto ~p~n\tpacket "
	   "~P~n",
	   [From, To, Packet, 8]),
    Host = To#jid.lserver,
    case gen_mod:is_loaded(Host, mod_gcm) of
	  ...
	  case lookup_cache(To) of
	    ...
	    DeviceList ->
		Offline =
		    ejabberd_hooks:run_fold(count_offline_messages, Host, 0,
					    [To#jid.luser, Host])
		      + 1,
		lists:foreach(fun ({ID, AppID, SendBody, SendFrom}) ->
				      ?DEBUG("lookup: ~p~n",
					     [{ID, AppID, SendBody, SendFrom}]),
				      do_send_offline_packet_notification(From,
									  To,
									  Packet,
									  ID,
									  AppID,
...
process_sm_iq(From, To,
    ...
    case {Type, SubEl} of
	  ...
	  case lookup_cache(To, SDeviceID) of
		...
		PushService = get_push_service(Host, To, AppID),
		ServiceJID = jlib:make_jid(<<"">>, PushService, <<"">>),
		if From#jid.lserver == ServiceJID#jid.lserver ->
	  ...
	  case lookup_cache(To, OldDeviceID) of
		...
		PushService = get_push_service(Host, To, AppID),
		ServiceJID = jlib:make_jid(<<"">>, PushService, <<"">>),
		?INFO_MSG("Replacing old id: ~s by canonical id: ~s",
...
device_reset_badge(Host, To, DeviceID, AppID, Badge) ->
    ?INFO_MSG("Sending reset badge (~p) push to ~p ~p",
	      [Badge, To, DeviceID]),
    PushService = get_push_service(Host, To, AppID),
    ServiceJID = jlib:make_jid(<<"">>, PushService, <<"">>),
...
resend_badge(To) ->
    ...
    case gen_mod:is_loaded(Host, mod_gcm) of
      true ->
	  case lookup_cache(To) of
	    false -> {error, <<"no cached data for the user">>};
	    DeviceList ->
		lists:foreach(
		  fun({ID, AppID, SendBody, SendFrom}) ->
			  ?DEBUG("lookup: ~p~n", [{ID, AppID, SendBody, SendFrom}]),
			  PushService = get_push_service(Host, To, AppID),
			  ServiceJID = jlib:make_jid(<<"">>, PushService, <<"">>),
			  Offline =
			      ejabberd_hooks:run_fold(count_offline_messages,
						      Host, 0,
						      [To#jid.luser,
						       Host]),
			  if Offline == 0 -> ok;
			     true ->
				  Badge =
				      jlib:integer_to_binary(Offline),
				  Packet1 = #xmlel{name =
						       <<"message">>,
						   attrs = [],
						   children =
      ...
      false -> {error, <<"mod_gcm is not loaded">>}
    end.
...
lookup_cache(JID) ->
	...
	mnesia -> lookup_cache_mnesia(JID);
	odbc -> lookup_cache_sql(JID)
    end.
...
lookup_cache_sql(JID) ->
    ...
    do_lookup_cache_sql(LServer,
			[<<"select device_id, app_id, send_body, "
			   "send_from from gcm_cache where username='">>,
			 Username, <<"'">>]).

lookup_cache(JID, DeviceID) ->
    case gen_mod:db_type(JID#jid.lserver, ?MODULE) of
      mnesia -> lookup_cache_mnesia(JID, DeviceID);
      odbc -> lookup_cache_sql(JID, DeviceID)
    end.

lookup_cache_mnesia(JID, DeviceID) ->
    #jid{luser = LUser, lserver = LServer} = JID,
    LUS = {LUser, LServer},
    do_lookup_cache_mnesia(#gcm_cache{device_id = DeviceID,
				      us = LUS, _ = '_'}).
...
lookup_cache_sql(JID, DeviceID) ->
    ...
    do_lookup_cache_sql(LServer,
			[<<"select device_id, app_id, send_body, "
			   "send_from from gcm_cache where username='">>,
			 Username, <<"' and device_id='">>, DeviceID, <<"'">>]).

do_lookup_cache_mnesia(MatchSpec) ->
    case mnesia:dirty_match_object(MatchSpec) of
      EntryList when is_list(EntryList) ->
	  lists:map(fun (#gcm_cache{device_id = DeviceID,
				    options = Options}) ->
			    AppID = proplists:get_value(appid, Options,
							<<"gcm.localhost">>),
			    SendBody = proplists:get_value(send_body, Options,
							   none),
			    SendFrom = proplists:get_value(send_from, Options,
							   true),
...
do_lookup_cache_sql(LServer, Query) ->
    case ejabberd_odbc:sql_query(LServer, Query) of
      {selected,
       [<<"device_id">>, <<"app_id">>, <<"send_body">>,
	<<"send_from">>],
       EntryList} ->
	  lists:map(fun ([SDeviceID, AppID, SSendBody,
			  SSendFrom]) ->
			    ...
			    {SDeviceID, AppID, SendBody, SendFrom}
		    end,
		    EntryList);
      _ -> false
    end.

store_cache(JID, DeviceID, AppID, SendBody, SendFrom,
	    TimeStamp) ->
    case gen_mod:db_type(JID#jid.lserver, ?MODULE) of
      mnesia ->
	  Options = [{appid, AppID}, {send_body, SendBody},
		     {send_from, SendFrom}, {timestamp, TimeStamp}],
	  store_cache_mnesia(JID, DeviceID, Options);
      odbc ->
	  store_cache_sql(JID, DeviceID, AppID, SendBody,
			  SendFrom)
    end.
...
store_cache_mnesia(JID, DeviceID, Options) ->
    #jid{luser = LUser, lserver = LServer} = JID,
    LUS = {LUser, LServer},
    R = #gcm_cache{us = LUS, device_id = DeviceID,
		   options = Options},
    case catch mnesia:dirty_read(gcm_cache, LUS) of
      [R] -> ok;
      _ ->
	  lists:foreach(fun (R1) -> mnesia:dirty_delete_object(R1)
			end,
			mnesia:dirty_index_read(gcm_cache, DeviceID,
						#gcm_cache.device_id)),
...
store_cache_sql(JID, DeviceID, AppID, SendBody,
    ...
    SSendBody = case SendBody of
		  all -> <<"A">>;
		  first_per_user -> <<"U">>;
		  first -> <<"F">>;
		  _ -> <<"-">>
		end,
    SSendFrom = case SendFrom of
		  jid -> <<"J">>;
		  username -> <<"U">>;
		  name -> <<"N">>;
		  _ -> <<"-">>
    ...
    F = fun () ->
		ejabberd_odbc:sql_query_t([<<"delete from gcm_cache where username='">>,
					   Username, <<"' and device_id='">>,
					   DeviceID, <<"';">>]),
		ejabberd_odbc:sql_query_t([<<"insert into gcm_cache(username, device_id, "
					     "app_id,                             "
					     "send_body, send_from) values ('">>,
					   Username, <<"', '">>, DeviceID,
					   <<"', '">>, SAppID, <<"', '">>,
					   SSendBody, <<"', '">>, SSendFrom,
					   <<"');">>])
...
update_cache(JID, OldDeviceID, NewDeviceID) ->
    case lookup_cache(JID, OldDeviceID) of
      [{_ID, AppID, SendBody, SendFrom}] ->
	  delete_cache(JID, OldDeviceID),
	  {MegaSecs, Secs, _MicroSecs} = now(),
	  TimeStamp = MegaSecs * 1000000 + Secs,
	  store_cache(JID, NewDeviceID, AppID, SendBody, SendFrom,
		      TimeStamp);
      _ -> ok
    end.
...
delete_cache(JID, DeviceID) ->
    case gen_mod:db_type(JID#jid.lserver, ?MODULE) of
      mnesia -> delete_cache_mnesia(JID, DeviceID);
      odbc -> delete_cache_sql(JID, DeviceID)
    end.
...
delete_cache_mnesia(JID, DeviceID) ->
    ...
    [mnesia:dirty_delete_object(Obj)
     || Obj
	    <- mnesia:dirty_match_object(#gcm_cache{device_id =
							DeviceID,
						    us = LUS, _ = '_'})].
...
delete_cache_sql(JID, DeviceID) ->
    ...
    ejabberd_odbc:sql_query(LServer,
			    [<<"delete from gcm_cache where username='">>,
			     Username, <<"' and device_id='">>, DeviceID,
			     <<"';">>]).

delete_cache(JID) ->
    case gen_mod:db_type(JID#jid.lserver, ?MODULE) of
      mnesia -> delete_cache_mnesia(JID);
      odbc -> delete_cache_sql(JID)
    end.
...
init_host(VHost, ValidEncryptedList) ->
    ...
    case lists:member(EncryptedHost, ValidEncryptedList) of
	  ...
	  case ?MAX_USERS of
	    0 -> true;
	    N ->
		ejabberd_auth:get_vh_registered_users_number(VHost) =< N
	  end;
      false ->
	  case str:chr(VHost, $.) of
	    0 -> false;
	    Pos ->
		init_host(str:substr(VHost, Pos + 1),
			  ValidEncryptedList)
	  end
    end.

get_tokens_by_jid(JIDString)
    when is_binary(JIDString) ->
    get_tokens_by_jid(jlib:string_to_jid(JIDString));
get_tokens_by_jid(JID = #jid{}) ->
...
apply_on_all_ios_for(JID, OnlineFun, OfflineFun)
    when is_binary(JID) ->
    apply_on_all_ios_for(jlib:string_to_jid(JID), OnlineFun,
			 OfflineFun);
apply_on_all_ios_for(JID = #jid{luser = User,
				lserver = Server},
		     OnlineFun, OfflineFun) ->
    case lookup_cache(JID) of
      false -> ok;
      Devices ->
	  AllGCMDeviceIds = [ID
			     || {ID, _AppID, _SendBody, _SendFrom} <- Devices],
	  Pids = [ejabberd_sm:get_session_pid(User, Server,
					      Resource)
		  || Resource
			 <- ejabberd_sm:get_user_resources(User, Server)],
	  OnlineGCMDeviceIDs = [{D, P}
				|| {D, P}
				       <- [{catch
					      ejabberd_c2s:get_ios_clientid(Pid),
					    Pid}
					   || Pid <- Pids],
				   is_binary(D)],
	  OfflineGCMDevices = lists:filter(fun (ID) ->
						   not
						     lists:keymember(ID, 1,
								     OnlineGCMDeviceIDs)
					   end,
					   AllGCMDeviceIds),
	  OnlineFun(OnlineGCMDeviceIDs),
	  OfflineFun(OfflineGCMDevices)
    end.
