...
-export([update_node_database/2, update_state_database/2, export/1]).
...
update_state_database(_Host, _ServerHost) ->
	...
	[stateid, items, affiliation, subscription] ->
	    ...
	    case mnesia:transaction(FNew) of
		{atomic, Result} ->
		    ?INFO_MSG("Pubsub state tables updated correctly: ~p", [Result]);
		{aborted, Reason} ->
		    ?ERROR_MSG("Problem updating Pubsub state tables:~n~p", [Reason])
	    end;
...
export(_Server) ->
    [{pubsub_item,
	    fun(_Host, #pubsub_item{itemid = {Itemid1, Itemid2},
		    ...
		    % TODO
		    % The call xml:element_to_binary(PayloadEl::{'xmlelement',_,_,_}) will never return since the success typing is
		    % ({'xmlcdata',binary()} | {'xmlel',binary(),[{binary(),binary()}],[{'xmlcdata',binary()} | {'xmlel',binary(),[any()],[any()]}]}) -> binary()
		    % we must convert xmlelement to xmlel first
