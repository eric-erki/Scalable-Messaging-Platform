...
-export([create_node/5, create_node/7, delete_node/3,
         ...
         tree_action/3, node_action/4, node_call/3]).
...
(
    [binary() | [{_,_}],...])
    -> {'ok',#state{server_host::binary(),host::binary(),nodetree::atom(),plugins::[any()]}}
).
...
init_plugins(Host, ServerHost, Opts) ->
    TreePlugin = tree(gen_mod:get_opt(nodetree, Opts,
                                      fun(A) when is_binary(A) -> A end, ?STDTREE)),
...
init_nodes(Host, ServerHost, _NodeTree, Plugins) ->
    case lists:member(<<"hometree_odbc">>, Plugins) of
        true ->
            ...
            create_node(Host, ServerHost, <<"/home/", ServerHost/binary>>, service_jid(Host), <<"hometree_odbc">>);
        false ->
            ok
    end.
...
send_loop(State) ->
    receive
        {presence, JID, Pid} ->
            ...
            lists:foreach(
                fun(PType) ->
                        Subs = case catch node_action(Host, PType,
                        ...
                        case get_option(Options, send_last_published_item) of
                            on_sub_and_presence ->
                                lists:foreach(fun(Resource) ->
                                        ...
                                        if Subscribed ->
                                            send_items(Owner, Node, Nidx, Type, LJID, last);
                                        true ->
                                            ok
                                        end
                                    end,
...
disco_identity(Host, Node, From) ->
    Action = fun (#pubsub_node{id = Nidx, type = Type, options = Options}) ->
...
disco_features(_Host, <<>>, _From) ->
    [?NS_PUBSUB | [feature(F) || F <- features(<<"pep">>)]];
disco_features(Host, Node, From) ->
    Action = fun (#pubsub_node{id = Nidx, type = Type, options = Options}) ->
            Owners = node_owners_call(Type, Nidx),
            case get_allowed_items_call(Host, Nidx, From, Type, Options, Owners) of
                {result, _} -> {result, [?NS_PUBSUB | [feature(F) || F <- features(<<"pep">>)]]};
                _ -> {result, []}
            end
    end,
    case transaction(Host, Node, Action, sync_dirty) of
                            ...
                            lists:foreach(fun
                                    ({#pubsub_node{options = Options,
                                        ...
                                        case get_option(Options, access_model) of
                                            presence ->
                                                Owners = node_owners(Host, PType, Nidx),
                                                case
                                                    lists:member(BJID, Owners) of
                                                    true ->
    ...
    spawn(fun () ->
                  lists:foreach(fun (PType) ->
                            ...
                            lists:foreach(fun
                                    ...
                                    ({#pubsub_node{nodeid = {H, N}, type = <<"hometree_odbc">>}, owner})
                                            when N == HomeTreeBase ->
                                        delete_node(H, N, Entity);
...
send_authorization_request(#pubsub_node{nodeid = {Host, Node}, type = Type, id = Nidx}, Subscriber) ->
    Lang = <<"en">>,
        ...
        {{value, {_, [Node]}},
         ...
         {value, {_, [SAllow]}}} ->
            ...
            Action = fun (#pubsub_node{type = Type, id = Nidx}) ->
                    ...
                    IsApprover = lists:member(FromLJID, node_owners_call(Type, Nidx)),
...
update_auth(Host, Node, Type, Nidx, Subscriber, Allow, Subscriptions) ->
    ...
    case Subscription of
        [{pending, SubId}] ->
            ...
            node_call(Type, set_subscriptions, [Nidx, Subscriber, NewSubscription, SubId]),
            send_authorization_approval(Host, Subscriber, Node, NewSubscription),
            {result, ok};
        _ ->
    ...
    case transaction(Host, Node, Action, transaction) of
        {result, {_, {Result, broadcast, Removed}}} ->
            lists:foreach(fun ({RNode, _RSubscriptions}) ->
                        ...
                        broadcast_removed_node(RH, RN, RNidx, RType, ROptions),
...
get_allowed_items_call(Host, Nidx, From, Type, Options, Owners, RSM) ->
    ...
    node_call(Type, get_items, [Nidx, From, AccessModel, PS, RG, undefined, RSM]).
...
send_items(Host, Node, Nidx, Type, LJID, Number) when Number > 0 ->
    Items = case node_action(Host, Type, get_last_items, [Nidx, LJID, Number]) of
        {result, All} -> All;
        _ -> []
    end,
    Stanza = items_event_stanza(Node, Items),
    send_items_stanza(Host, Node, LJID, Stanza);
    ...
    case Result of
        {ok, Affiliations} ->
            Entities = lists:flatmap(fun
                        ({_, none}) ->
                            [];
                        ({#pubsub_node{nodeid = {_, NodeId}}, Affiliation}) ->
...
set_affiliations(Host, Node, From, EntitiesEls) ->
    ...
    case Entities of
        error ->
            {error, ?ERR_BAD_REQUEST};
        _ ->
            Action = fun (#pubsub_node{type = Type, id = Nidx} = N) ->
                    Owners = node_owners_call(Type, Nidx),
...
read_sub(Subscriber, Node, Nidx, SubId, Lang) ->
    Children = case pubsub_subscription_odbc:get_subscription(Subscriber, Nidx, SubId) of
        {error, notfound} ->
            [];
...
write_sub(Subscriber, Nidx, SubId, Options) ->
    pubsub_subscription_odbc:set_subscription(Subscriber, Nidx, SubId, Options),
    {result, []}.
                        ...
                        true ->
                            Result = lists:foldl(fun ({JID, Subscription, SubId}, Acc) ->
                                            case
                                                node_call(Type,
                                                          set_subscriptions,
                                                          [Nidx, JID, Subscription, SubId])
                                            of
                                                {error, Err} ->
                                                    [{error, Err} | Acc];
                                                _ ->
                                                    Notify(JID, Subscription, SubId),
                                                    Acc
                                            end
                                    end,
                                    [], Entities),
...
event_stanza(Event, EvAttr, Entries) ->
    event_stanza_with_els([#xmlel{name = Event, attrs = EvAttr,
                                  children = [#xmlel{name = Entry, attrs = EnAttr}
...
broadcast_publish_item(Host, Node, Nidx, Type, NodeOptions, ItemId, Publisher, Payload, Removed) ->
    ...
    case Removed of
        [] ->
            {result,
             broadcast(Host, Node, Nidx, Type, NodeOptions, items, true, Publish, true)};
        _ ->
...
broadcast_retract_items(Host, Node, Nidx, Type, NodeOptions, ItemIds) ->
    broadcast_retract_items(Host, Node, Nidx, Type, NodeOptions, ItemIds, notify_retract).

broadcast_retract_items(_Host, _Node, _Nidx, _Type, _NodeOptions, [], _) ->
    {result, false};
broadcast_retract_items(Host, Node, Nidx, Type, NodeOptions, ItemIds, false) ->
    broadcast_retract_items(Host, Node, Nidx, Type, NodeOptions, ItemIds, notify_retract);
broadcast_retract_items(Host, Node, Nidx, Type, NodeOptions, ItemIds, Notify) ->
    Stanza = event_stanza(<<"items">>, nodeAttr(Node),
                          [{<<"retract">>, itemAttr(Rid)} || Rid <- ItemIds]),
    {result,
     broadcast(Host, Node, Nidx, Type, NodeOptions, items, Notify, Stanza, true)}.

broadcast_purge_node(Host, Node, Nidx, Type, NodeOptions) ->
    Stanza = event_stanza(<<"purge">>, nodeAttr(Node)),
    {result,
     broadcast(Host, Node, Nidx, Type, NodeOptions, nodes, notify_retract, Stanza, false)}.

broadcast_removed_node(Host, Node, Nidx, Type, NodeOptions) ->
    Stanza = event_stanza(<<"delete">>, nodeAttr(Node)),
    {result,
     broadcast(Host, Node, Nidx, Type, NodeOptions, nodes, notify_delete, Stanza, false)}.

broadcast_created_node(Host, Node, Nidx, Type, NodeOptions) ->
    Stanza = event_stanza(<<"create">>, nodeAttr(Node)),
    {result,
     broadcast(Host, Node, Nidx, Type, NodeOptions, nodes, true, Stanza, true)}.

broadcast_config_notification(Host, Node, Nidx, Type, NodeOptions, Lang) ->
    Stanza = case get_option(NodeOptions, deliver_payloads) of
        true ->
            event_stanza(<<"configuration">>, nodeAttr(Node), <<"x">>,
                         [{<<"xmlns">>, ?NS_XDATA},
                          {<<"type">>, <<"result">>}],
    ...
    {result,
     broadcast(Host, Node, Nidx, Type, NodeOptions, nodes, notify_config, Stanza, false)}.

broadcast(Host, Node, Nidx, Type, NodeOptions, Notify, Stanzas) ->
    Subs = node_subscriptions(Host, Node, Nidx, Type, NodeOptions, Notify),
    Result = [broadcast(Host, Node, Nidx, Type, NodeOptions, Subs, Stanza, SHIM)
              || {Cond, Stanza, SHIM} <- Stanzas, Cond =:= true],
    lists:member(true, Result).

broadcast(Host, Node, Nidx, Type, NodeOptions, Notify, true, Stanza, SHIM) ->
    Subs = node_subscriptions(Host, Node, Nidx, Type, NodeOptions, Notify),
    broadcast(Host, Node, Nidx, Type, NodeOptions, Subs, Stanza, SHIM);
broadcast(_Host, _Node, _Nidx, _Type, _NodeOptions, _Notify, false, _Stanza, _SHIM) ->
    false;
broadcast(Host, Node, Nidx, Type, NodeOptions, Notify, Condition, Stanza, SHIM) ->
    broadcast(Host, Node, Nidx, Type, NodeOptions, Notify, get_option(NodeOptions, Condition), Stanza, SHIM).

broadcast({U, S, R}, Node, Nidx, Type, NodeOptions, Subscriptions, Stanza, SHIM) ->
    broadcast(S, Node, Nidx, Type, NodeOptions, Subscriptions, Stanza, SHIM)
    or
...
broadcast(_Host, _Node, _Nidx, _Type, _NodeOptions, [], _Stanza, _SHIM) ->
    false;
broadcast(Host, _Node, _Nidx, _Type, NodeOptions, Subscriptions, Stanza, SHIM) ->
...
node_subscriptions(Host, Node, Nidx, Type, _NodeOptions, Notify) ->
    case Type of
...
node_subscriptions_bare(Host, Node, Nidx, Type) ->
    case node_action(Host, Type, get_node_subscriptions, [Nidx]) of
        {result, Subs} ->
            SubsByJid = lists:foldl(fun ({JID, subscribed, SubId}, Acc) ->
...
node_subscriptions_full(Host, Node, NotifyType) ->
    Action = fun () ->
            ...
            {result, [{Depth, [{N, sub_with_options(N)} || N <- Nodes]}
                            ...
                            true ->
                                case state_can_deliver(LJID, SubOptions) of
                                    [] ->
                                        {JIDs, Recipients};
                                    JIDsToDeliver ->
                                        lists:foldl(fun (JIDToDeliver, {JIDsAcc, RecipientsAcc}) ->
                                                    case lists:member(JIDToDeliver, JIDs) of
                                                        ...
                                                        true ->
                                                            {_, {JIDToDeliver, NodeName1, SubIds}} =
                                                                         lists:keysearch(JIDToDeliver, 1,
                                                                                         RecipientsAcc),
                                                            {JIDsAcc,
...
sub_with_options(#pubsub_node{type = Type, id = Nidx}) ->
    case node_call(Type, get_node_subscriptions, [Nidx])
    of
        {result, Subs} ->
            lists:foldl(fun
...
sub_with_options(JID, Nidx, SubId) ->
    case pubsub_subscription_odbc:get_subscription(JID, Nidx, SubId) of
...
node_owners_call(Type, Nidx) ->
    case node_call(Type, get_node_affiliations, [Nidx]) of
      {result, Affiliations} ->
          lists:foldl(fun ({LJID, owner}, Acc) -> [LJID | Acc];
                          (_, Acc) -> Acc
                      end,
...
tree(Name) ->
    jlib:binary_to_atom(<<"nodetree_", Name/binary, "_odbc">>).

plugin(Name) ->
    jlib:binary_to_atom(<<"node_", Name/binary, "_odbc">>).
plugins(Host) ->
...
purge_offline({User, Server, _} = LJID) ->
                    ...
                    case lists:member(<<"retrieve-affiliations">>, features(Type)) of
                        false ->
                            {{error,
                             ...
                             Acc};
                        true ->
                            {result, Affiliations} = node_action(Host, Type,
                                                                 get_entity_affiliations,
                                                                 [Host, LJID]),
    ...
    case Result of
        {ok, Affiliations} ->
            lists:foreach(fun
                    ({#pubsub_node{nodeid = {_, Node}, options = Options, type = Type}, Affiliation})
