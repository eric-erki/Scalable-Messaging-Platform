...
do_start(Level) ->
    ...
    LogRotateDate = get_string_env(log_rotate_date, "$D0"),
    LogRotateSize = get_integer_env(log_rotate_size, 0),
    ...
    Handlers = case get_string_env(syslog, undefined) of
        undefined ->
            [{lager_file_backend, [{file, ConsoleLog}, {level, Level},
                                   {date, LogRotateDate},
             ...
             {lager_file_backend, [{file, ErrorLog}, {level, error},
                                   ...
                                   {size, LogRotateSize}]}];
        Facility ->
            [{lager_syslog_backend, ["ejabberd", iolist_to_binary(Facility), Level]}]
    end,
    application:set_env(
        lager, handlers, [{lager_console_backend, Level} | Handlers]),
