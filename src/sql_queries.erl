...
-export([get_db_type/0, update_t/4, update/5,
...
add_roomhistory_sql(Room, Nick, Packet, HaveSubject,
		    Timestamp, Size) ->
    ?SQL("insert into room_history(room, nick, "
         "packet, have_subject, timestamp, size) "
         "values (%(Room)s, %(Nick)s, %(Packet)s, %(HaveSubject)b,"
         " %(Timestamp)d, %(Size)d);").

clear_and_add_roomhistory(LServer, Room, Queries) ->
    Q = ?SQL("delete from room_history where room = %(Room)s;"),
    ejabberd_sql:sql_transaction(LServer, [Q | Queries]).
