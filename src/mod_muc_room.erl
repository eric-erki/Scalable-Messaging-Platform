...
start(StateName, StateData) ->
    ...
    (?GEN_FSM):start(?MODULE, [StateName, StateData],
		     ?FSMOPTS).
	  ...
	  case fxml:get_attr_s(<<"type">>, Attrs) of
		...
		MinMessageInterval =
		    trunc(gen_mod:get_module_opt(StateData#state.server_host,
						 ...
						 fun(MMI) when is_number(MMI), MMI>=0 -> MMI end, 0)
                          * 1000000),
...
terminate({migrated, Node}, StateName, StateData) ->
    ...
    case catch ets:match_object(muc_online_users,
			#muc_online_users{room = StateData#state.room,
					  host = StateData#state.host,
					  _ = '_'})
	of
	Rs when is_list(Rs) ->
	    JIDs = [jid:make(U, S, R)
		    || #muc_online_users{us = {U, S}, resource = R} <- Rs],
	    ejabberd_cluster:call(Node, ?MODULE, tab_online_users, [JIDs, StateData]);
	_ ->
	    ok
    end,
...
make_opts(StateData) ->
    ...
    case Subscribers of
	[] -> Opts4;
	_ -> [{subscribers, Subscribers}|Opts4]
    end.
...
tab_online_users(JIDs, StateData) ->
    [tab_add_online_user(JID, StateData) || JID <- JIDs],
    ok.
