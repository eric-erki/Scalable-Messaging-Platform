...
\newcommand{\modhttplog}{\module{mod\_http\_log}}
\newcommand{\modhttpnotify}{\module{mod\_http\_notify}}
\newcommand{\modhttpadmin}{\module{mod\_http\_admin}}
\newcommand{\modhttpwatchdog}{\module{mod\_http\_watchdog}}
...
\newcommand{\dbtype}{\titem{db\_type: mnesia|odbc|riak} \ind{options!dbtype}
  ...
  If \term{odbc} or \term{riak} value is defined, make sure you have defined the database,
  see~\ref{database}.
}
...
\begin{description}
	...
	See section~\ref{database} for more information.
        ...
        If you provided the configure option \term{--enable-debug=KEY} then KEY will be used
        as an encryption key and beam files will be compiled with \term{debug\_info\_key} enabled.
...
\begin{description}
  \titem{\texttt{ejabberd\_c2s}}
    ...
    \texttt{protocol\_options}, \texttt{cafile}, \texttt{tls\_verify}, \texttt{ocsp},
...
\begin{description}
  ...
  \titem{cafile: Path} Full path to a file containing root CA certificates in PEM format.
  This option is only used in conjunction with \option{tls\_verify} option.
  \titem{ocsp: true|false} If \option{tls\_verify} is enabled, this option enables SASL EXTERNAL
  authentication via OCSP protocol (RFC6960) in favor of CRL protocol (RFC5280). The default is \term{false},
  meaning that CRL protocol is in use by default.
...
\end{verbatim}
  ...
  \titem{tls\_verify: true|false}
  Set SASL EXTERNAL authentication as described in \xepref{0178}. When enabled another
  option \option{cafile} should specify the file with root CA certificates in PEM format.
  All client certificates should be signed by one of these root CA certificates and should
  contain the corresponding JID(s) in \term{subjectAltName} field as described in
  \footahref{https://tools.ietf.org/html/rfc6120\#section-13.7.1.4}{RFC 6120: XMPP Core}.
  If \option{cafile} is not defined, system CA certificate storage will be consulted
  which in the most cases will be too broad and, thus, insecure. Note that a user
  should be still registered in the system in order for this type of authentication to work.
  ...
  \titem{accept\_interval: Interval | \{linear, [I1, T1, T2, I2]\}}
  \ind{options!accept\_interval} This option specifies a minimal interval
  between accepting connections.  It can be a single integer specifying the
  interval in milliseconds, or a set of 4 numbers.  The number \term{I1}
  sets the initial interval in milliseconds after \ejabberd{} startup, which
  will last for \term{T1} seconds, and then during \term{T2} seconds it will
  change linearly to \term{I2} milliseconds.  It's useful for handling clients
  reconnections after \ejabberd{} restarts.
\end{description}

There are some additional global options that can be specified in the ejabberd configuration file (outside \term{listen}):
\begin{description}
  \titem{c2s\_tls\_verify: true|false} Same as the listening option \term{tls\_verify}
  but can be applied to a different virtual hosts if used within \option{host\_config}.
  ...
  \titem{c2s\_hibernate: TimeoutMs|hibernate}
  Inactivity timeout in milliseconds before the c2s processes will be hibernated.
  The default value is \term{90000}.
  \titem{receiver\_hibernate: TimeoutMs|hibernate}
  Inactivity timeout in milliseconds before the receiver processes will be hibernated.
  The default value is \term{90000}.
  \titem{max\_fsm\_queue: Size}
    This option specifies the maximum number of elements in the queue of the FSM
    (Finite State Machine).
...
The default value is \term{1}. Note that this option doesn't take into consideration
the number of all connections with a specific remote server on all nodes in
a cluster. That is, for example, if there is no S2S connection with the server on local node,
the one will be created even if such S2S connections already exist on another nodes.
...
\begin{table}[H]
  ...
  \begin{tabular}{|l|l|l|}
    ...
    \hline \ahrefloc{modhttplog}{\modhttplog{}} & Log XMPP packets over HTTP &  \\
    \hline \ahrefloc{modhttpnotify}{\modhttpnotify{}} & Send HTTP notifications about some events &  \\
    \hline \ahrefloc{modhttpadmin}{\modhttpadmin{}} & Admin REST API &  \\
    \hline \ahrefloc{modhttpwatchdog}{\modhttpwatchdog{}} & Simple watchdog through REST call &  \\
...
\makesubsection{modhttplog}{\modhttplog{}}
\ind{modules!\modhttplog{}}\ind{modhttplog}

This module logs XMPP packets of a given session over HTTP.

To use this feature, enable the module:
\begin{verbatim}
modules:
  ...
  mod_http_log: {}
...
listen:
  ...
  ...
\end{verbatim}
This page is designed to be used by web browsers, or raw terminal clients like curl.
Example, you can get XMPP logs with curl at following url \verb|http://example.org:5280/log/user@example.org|

If you share this module with other public use (like BOSH), it's best to consider put this module in a dedicated
http listener on a private port.

Options:
\begin{description}
  \titem{access: AccessName} \ind{options!access}This option can be used to specify who
  may use this feature. (default value: \term{all}).
\end{description}
\begin{verbatim}
modules:
  ...
...
\makesubsection{modhttpnotify}{\modhttpnotify{}}
\ind{modules!\modhttpnotify{}}\ind{modhttpnotify}

This module sends HTTP notifications to an external service on some events.
Actual implementation logs presence available, unavailable, change, and offline packets.
Information is encoded as \verb|application/x-www-form-urlencoded|.

To enable this feature, enable the module:
\begin{verbatim}
modules:
  ...
  mod_http_notify:
...
Options:
\begin{description}
  \titem{url: Url} \ind{options!url}This option must be used to specify server to notify.
  \titem{auth\_token: Secret} \ind{options!auth\_token}This option must be used to define secret token
  to authenticate the query.
\end{description}

\makesubsection{modhttpadmin}{\modhttpadmin{}}
\ind{modules!\modhttpadmin{}}\ind{modhttpadmin}

This module provides administration commands through REST API using JSON for data and result.
The set of available commands is the same which is available through \term{ejabberdctl}\ind{ejabberdctl} command.

To enable administration REST API, enable the admin modules:
\begin{verbatim}
modules:
  ...
  mod_http_admin: {}
...
listen:
  ...
  ...
\end{verbatim}
This page is designed to be used by REST library, but can also be used by web browsers, or raw terminal clients like curl.
Example, you can get server status with curl at following url \verb|http://example.org:5280/api/server_info|

If you share this module with other public use (like BOSH), it's best to consider put this module in a dedicated
http listener on a private port.

Command parameters can be passed as json data or as url encoded parameter.
Example:
\begin{verbatim}
curl -X POST --data '["test1","example.org"]' http://example.org:5280/api/user_info
curl -X POST --data '{"user":"test1","server":"example.org"}' http://example.org:5280/api/user_info
curl "http://example.org:5280/api/user_info?user=test1&server=example.org"
\end{verbatim}
...
\makesubsection{modhttpwatchdog}{\modhttpwatchdog{}}
\ind{modules!\modhttpwatchdog{}}\ind{modhttpwatchdog}

This module implements a simple watchdog user can trigger with a REST call. The module does two tests:
connection to listener, and full XMPP authentication.
...
listen:
  ...
  ...
\end{verbatim}
You can trigger the watchdog at following url \verb|http://example.org:5280/status|

Options:
\begin{description}
  \titem{test\_domain: TestDomain} \ind{options!test\_domain}This option defines the XMPP domain to connect, this
   is usually the xmpp vhost that runs this module.
  \titem{test\_user: TestUser} \ind{options!test\_user}This option defines the login of the test user.
  \titem{test\_pass: TestPass} \ind{options!test\_pass}This option defines the password of the test user.
\end{description}
...
\titem{default\_room\_options: \{OptionName: OptionValue\}} \ind{options!default\_room\_options}
  ...
  \titem{hibernate\_timeout: Seconds} Remove empty rooms from RAM.
  The feature is disabled by default. Note that hibernated room is not available in
  service discovery.
...
This means that all messages sent to an offline user will be stored on the server until
hat user comes online again. Message is delivered by the server at user reconnection.
Thus it is very similar to how email works.
Message can also include an expiration attribute (\xepref{0023}) to prevent server to deliver
outdated message at user reconnection.
Note that \term{ejabberdctl}\ind{ejabberdctl} has a command to delete expired messages
(see section~\ref{ejabberdctl}).
...
\begin{description}
  ...
  \titem{pool\_size: N}\ind{options!pool\_size} Size of the worker pool for offline storage. Default is 16.
\end{description}
...
\dbtype
  ...
  \titem{use\_cache: false|true} \ind{options!userostercache}
  Enables caching of users rosters in RAM. The option is only available when
  \term{db\_type} is set to \term{odbc}.
  This option is disabled by default. Use this option with extreme care in
  clustered environment: if the same user is connected to several nodes
  then roster modifications won't be propagated to other nodes' caches.
  You can use small \term{cache\_life\_time} values to minimize probability of
  such situation though.
  \titem{cache\_life\_time: Seconds} Sets cache life time in seconds.
  Implies that caching is enabled. The default is 5 seconds. Such small default
  value reflects the common usage: repeatable roster reads on user login.
  \titem{cache\_size: Integer|unlimited}
  This option limits the maximum number of users rosters kept in the cache.
  Big cache size leads to RAM consumption.
  Small cache size leads to disc/network consumption.
  The option implies that caching is enabled. The default is 1000.
...
\makesection{ssldist}{Secure internode connections}
It is possible to protect internode connections using SSL.
You need to create a ``boot'' file for doing that. The script \term{tools/start\_ssl.sh}
can do that for you:
\begin{verbatim}
$ tools/start_ssl.sh
\end{verbatim}
The script will create \term{start\_ssl.boot} file in the current directory. Now
you should use this file and your SSL certificate to run the emulator:
\begin{verbatim}
$ erl -boot start_ssl \
      -proto_dist inet_tls \
...
By default \ejabberd{} rotates the log files every day at midnight.
...
\begin{description}
  ...
  The default value is \term{"\$D0"} (every day at midnight).
\end{description}

\ejabberd{} can also rotates the log files when they get grown above a certain size.
...
\begin{description}
  ...
  The default value is 0 (no rotation given log size).
\end{description}
