...
\titem{\{access, AccessName\}}\ind{options!access}
  ...
  The default value is \term{all}.
\titem{\{limits, [\{SenderType, StanzaType, Number\}]\}}\ind{options!access}
  Specify a list of custom limits which override the default ones defined in \xepref{0033}.
  SenderType can have values: local or remote.
  StanzaType can have values: message or presence.
  Number can be a positive integer or the key word \term{infinite}.
...
\item \term{broadcast}: send the packet to all the nodes.
\end{itemize}
