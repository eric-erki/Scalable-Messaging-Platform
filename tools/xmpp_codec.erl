...
decode({xmlel, _name, _attrs, _} = _el) ->
    case {_name, get_attr(<<"xmlns">>, _attrs)} of
	  ...
	  decode_mam_prefs(<<"urn:xmpp:mam:tmp">>, _el);
      {<<"always">>, <<"urn:xmpp:mam:tmp">>} ->
	  decode_mam_always(<<"urn:xmpp:mam:tmp">>, _el);
      {<<"never">>, <<"urn:xmpp:mam:tmp">>} ->
	  decode_mam_never(<<"urn:xmpp:mam:tmp">>, _el);
      {<<"jid">>, <<"urn:xmpp:mam:tmp">>} ->
	  decode_mam_jid(<<"urn:xmpp:mam:tmp">>, _el);
      {<<"result">>, <<"urn:xmpp:mam:tmp">>} ->
	  decode_mam_result(<<"urn:xmpp:mam:tmp">>, _el);
      {<<"archived">>, <<"urn:xmpp:mam:tmp">>} ->
	  decode_mam_archived(<<"urn:xmpp:mam:tmp">>, _el);
      {<<"query">>, <<"urn:xmpp:mam:tmp">>} ->
	  decode_mam_query(<<"urn:xmpp:mam:tmp">>, _el);
      {<<"with">>, <<"urn:xmpp:mam:tmp">>} ->
	  decode_mam_with(<<"urn:xmpp:mam:tmp">>, _el);
      {<<"end">>, <<"urn:xmpp:mam:tmp">>} ->
	  decode_mam_end(<<"urn:xmpp:mam:tmp">>, _el);
      {<<"start">>, <<"urn:xmpp:mam:tmp">>} ->
	  decode_mam_start(<<"urn:xmpp:mam:tmp">>, _el);
      ...
      {<<"standby">>, <<"p1:rebind">>} ->
	  decode_p1_standby(<<"p1:rebind">>, _el);
      {<<"rebind">>, <<"p1:rebind">>} ->
	  decode_p1_rebind(<<"p1:rebind">>, _el);
      {<<"sid">>, <<"p1:rebind">>} ->
	  decode_p1_rebind_sid(<<"p1:rebind">>, _el);
      {<<"jid">>, <<"p1:rebind">>} ->
	  decode_p1_rebind_jid(<<"p1:rebind">>, _el);
      {<<"failure">>, <<"p1:rebind">>} ->
	  decode_p1_rebind_failure(<<"p1:rebind">>, _el);
      {<<"push">>, <<"p1:push">>} ->
	  decode_p1_push(<<"p1:push">>, _el);
      {<<"status">>, <<"p1:push">>} ->
	  decode_p1_push_status(<<"p1:push">>, _el);
      {<<"session">>, <<"p1:push">>} ->
	  decode_p1_push_session(<<"p1:push">>, _el);
      {<<"keepalive">>, <<"p1:push">>} ->
	  decode_p1_push_keepalive(<<"p1:push">>, _el);
      {<<"stream:features">>,
       <<"http://etherx.jabber.org/streams">>} ->
...
decode_mam_prefs(__TopXMLNS,
		 {xmlel, <<"prefs">>, _attrs, _els}) ->
    {Never, Always} = decode_mam_prefs_els(__TopXMLNS, _els,
					   [], []),
    Default = decode_mam_prefs_attrs(__TopXMLNS, _attrs,
				     undefined),
    {mam_prefs, Default, Always, Never}.

decode_mam_prefs_els(__TopXMLNS, [], Never, Always) ->
    {Never, Always};
decode_mam_prefs_els(__TopXMLNS,
    ...
    if _xmlns == <<>>; _xmlns == __TopXMLNS ->
	   decode_mam_prefs_els(__TopXMLNS, _els, Never,
				decode_mam_always(__TopXMLNS, _el));
       true ->
	   decode_mam_prefs_els(__TopXMLNS, _els, Never, Always)
    end;
decode_mam_prefs_els(__TopXMLNS,
    ...
    _xmlns = get_attr(<<"xmlns">>, _attrs),
    if _xmlns == <<>>; _xmlns == __TopXMLNS ->
	   decode_mam_prefs_els(__TopXMLNS, _els,
				decode_mam_never(__TopXMLNS, _el), Always);
       true ->
	   decode_mam_prefs_els(__TopXMLNS, _els, Never, Always)
    end;
decode_mam_prefs_els(__TopXMLNS, [_ | _els], Never,
		     Always) ->
    decode_mam_prefs_els(__TopXMLNS, _els, Never, Always).
...
decode_mam_always(__TopXMLNS,
		  {xmlel, <<"always">>, _attrs, _els}) ->
    Jids = decode_mam_always_els(__TopXMLNS, _els, []),
    Jids.

decode_mam_always_els(__TopXMLNS, [], Jids) ->
    lists:reverse(Jids);
decode_mam_always_els(__TopXMLNS,
		      [{xmlel, <<"jid">>, _attrs, _} = _el | _els], Jids) ->
    _xmlns = get_attr(<<"xmlns">>, _attrs),
    if _xmlns == <<>>; _xmlns == __TopXMLNS ->
	   decode_mam_always_els(__TopXMLNS, _els,
				 case decode_mam_jid(__TopXMLNS, _el) of
				   [] -> Jids;
				   _new_el -> [_new_el | Jids]
				 end);
       true -> decode_mam_always_els(__TopXMLNS, _els, Jids)
    end;
decode_mam_always_els(__TopXMLNS, [_ | _els], Jids) ->
    decode_mam_always_els(__TopXMLNS, _els, Jids).
...
decode_mam_never(__TopXMLNS,
		 {xmlel, <<"never">>, _attrs, _els}) ->
    Jids = decode_mam_never_els(__TopXMLNS, _els, []), Jids.

decode_mam_never_els(__TopXMLNS, [], Jids) ->
    lists:reverse(Jids);
decode_mam_never_els(__TopXMLNS,
		     [{xmlel, <<"jid">>, _attrs, _} = _el | _els], Jids) ->
    _xmlns = get_attr(<<"xmlns">>, _attrs),
    if _xmlns == <<>>; _xmlns == __TopXMLNS ->
	   decode_mam_never_els(__TopXMLNS, _els,
				case decode_mam_jid(__TopXMLNS, _el) of
				  [] -> Jids;
				  _new_el -> [_new_el | Jids]
				end);
       true -> decode_mam_never_els(__TopXMLNS, _els, Jids)
    end;
decode_mam_never_els(__TopXMLNS, [_ | _els], Jids) ->
    decode_mam_never_els(__TopXMLNS, _els, Jids).
...
decode_mam_jid(__TopXMLNS,
	       {xmlel, <<"jid">>, _attrs, _els}) ->
    Cdata = decode_mam_jid_els(__TopXMLNS, _els, <<>>),
    Cdata.

decode_mam_jid_els(__TopXMLNS, [], Cdata) ->
    decode_mam_jid_cdata(__TopXMLNS, Cdata);
decode_mam_jid_els(__TopXMLNS,
		   [{xmlcdata, _data} | _els], Cdata) ->
    decode_mam_jid_els(__TopXMLNS, _els,
		       <<Cdata/binary, _data/binary>>);
decode_mam_jid_els(__TopXMLNS, [_ | _els], Cdata) ->
    decode_mam_jid_els(__TopXMLNS, _els, Cdata).
...
decode_mam_result(__TopXMLNS,
		  {xmlel, <<"result">>, _attrs, _els}) ->
    __Els = decode_mam_result_els(__TopXMLNS, _els, []),
    {Queryid, Id} = decode_mam_result_attrs(__TopXMLNS,
					    _attrs, undefined, undefined),
    {mam_result, Queryid, Id, __Els}.

decode_mam_result_els(__TopXMLNS, [], __Els) ->
    lists:reverse(__Els);
decode_mam_result_els(__TopXMLNS,
		      [{xmlel, _, _, _} = _el | _els], __Els) ->
    case is_known_tag(_el) of
      true ->
	  decode_mam_result_els(__TopXMLNS, _els,
				[decode(_el) | __Els]);
      false -> decode_mam_result_els(__TopXMLNS, _els, __Els)
    end;
decode_mam_result_els(__TopXMLNS, [_ | _els], __Els) ->
    decode_mam_result_els(__TopXMLNS, _els, __Els).
...
decode_mam_archived(__TopXMLNS,
...
decode_mam_query(__TopXMLNS,
		 {xmlel, <<"query">>, _attrs, _els}) ->
    {End, Start, With, Rsm} =
	decode_mam_query_els(__TopXMLNS, _els, undefined,
			     undefined, undefined, undefined),
    Id = decode_mam_query_attrs(__TopXMLNS, _attrs,
				undefined),
    {mam_query, Id, Start, End, With, Rsm}.

decode_mam_query_els(__TopXMLNS, [], End, Start, With,
		     Rsm) ->
    {End, Start, With, Rsm};
decode_mam_query_els(__TopXMLNS,
    ...
    _xmlns = get_attr(<<"xmlns">>, _attrs),
    if _xmlns == <<>>; _xmlns == __TopXMLNS ->
	   decode_mam_query_els(__TopXMLNS, _els, End,
				decode_mam_start(__TopXMLNS, _el), With, Rsm);
       true ->
	   decode_mam_query_els(__TopXMLNS, _els, End, Start, With,
				Rsm)
    end;
decode_mam_query_els(__TopXMLNS,
    ...
    _xmlns = get_attr(<<"xmlns">>, _attrs),
    if _xmlns == <<>>; _xmlns == __TopXMLNS ->
	   decode_mam_query_els(__TopXMLNS, _els,
				decode_mam_end(__TopXMLNS, _el), Start, With,
				Rsm);
       true ->
	   decode_mam_query_els(__TopXMLNS, _els, End, Start, With,
				Rsm)
...
decode_mam_query_els(__TopXMLNS,
    ...
    _xmlns = get_attr(<<"xmlns">>, _attrs),
    if _xmlns == <<>>; _xmlns == __TopXMLNS ->
	   decode_mam_query_els(__TopXMLNS, _els, End, Start,
				decode_mam_with(__TopXMLNS, _el), Rsm);
       true ->
	   decode_mam_query_els(__TopXMLNS, _els, End, Start, With,
				Rsm)
    end;
decode_mam_query_els(__TopXMLNS,
    ...
    if _xmlns == <<"http://jabber.org/protocol/rsm">> ->
	   decode_mam_query_els(__TopXMLNS, _els, End, Start, With,
				decode_rsm_set(_xmlns, _el));
       true ->
	   decode_mam_query_els(__TopXMLNS, _els, End, Start, With,
				Rsm)
    end;
decode_mam_query_els(__TopXMLNS, [_ | _els], End, Start,
		     With, Rsm) ->
    decode_mam_query_els(__TopXMLNS, _els, End, Start, With,
			 Rsm).
...
decode_mam_with(__TopXMLNS,
		{xmlel, <<"with">>, _attrs, _els}) ->
    Cdata = decode_mam_with_els(__TopXMLNS, _els, <<>>),
    Cdata.

decode_mam_with_els(__TopXMLNS, [], Cdata) ->
    decode_mam_with_cdata(__TopXMLNS, Cdata);
decode_mam_with_els(__TopXMLNS,
		    [{xmlcdata, _data} | _els], Cdata) ->
    decode_mam_with_els(__TopXMLNS, _els,
			<<Cdata/binary, _data/binary>>);
decode_mam_with_els(__TopXMLNS, [_ | _els], Cdata) ->
    decode_mam_with_els(__TopXMLNS, _els, Cdata).
...
decode_mam_end(__TopXMLNS,
	       {xmlel, <<"end">>, _attrs, _els}) ->
    Cdata = decode_mam_end_els(__TopXMLNS, _els, <<>>),
    Cdata.

decode_mam_end_els(__TopXMLNS, [], Cdata) ->
    decode_mam_end_cdata(__TopXMLNS, Cdata);
decode_mam_end_els(__TopXMLNS,
		   [{xmlcdata, _data} | _els], Cdata) ->
    decode_mam_end_els(__TopXMLNS, _els,
		       <<Cdata/binary, _data/binary>>);
decode_mam_end_els(__TopXMLNS, [_ | _els], Cdata) ->
    decode_mam_end_els(__TopXMLNS, _els, Cdata).
...
decode_mam_start(__TopXMLNS,
		 {xmlel, <<"start">>, _attrs, _els}) ->
    Cdata = decode_mam_start_els(__TopXMLNS, _els, <<>>),
    Cdata.

decode_mam_start_els(__TopXMLNS, [], Cdata) ->
    decode_mam_start_cdata(__TopXMLNS, Cdata);
decode_mam_start_els(__TopXMLNS,
		     [{xmlcdata, _data} | _els], Cdata) ->
    decode_mam_start_els(__TopXMLNS, _els,
			 <<Cdata/binary, _data/binary>>);
decode_mam_start_els(__TopXMLNS, [_ | _els], Cdata) ->
    decode_mam_start_els(__TopXMLNS, _els, Cdata).
...
decode_rsm_set(__TopXMLNS,
	       {xmlel, <<"set">>, _attrs, _els}) ->
    {After, Last, First, Count, Before, Max, Index} =
	decode_rsm_set_els(__TopXMLNS, _els, undefined,
			   undefined, undefined, undefined, undefined,
			   undefined, undefined),
    {rsm_set, After, Before, Count, First, Index, Last,
     Max}.

decode_rsm_set_els(__TopXMLNS, [], After, Last, First,
...
decode_rsm_set_els(__TopXMLNS,
    ...
    _xmlns = get_attr(<<"xmlns">>, _attrs),
    if _xmlns == <<>>; _xmlns == __TopXMLNS ->
	   decode_rsm_set_els(__TopXMLNS, _els,
			      decode_rsm_after(__TopXMLNS, _el), Last, First,
			      Count, Before, Max, Index);
       true ->
	   decode_rsm_set_els(__TopXMLNS, _els, After, Last, First,
			      Count, Before, Max, Index)
    end;
decode_rsm_set_els(__TopXMLNS,
    ...
    if _xmlns == <<>>; _xmlns == __TopXMLNS ->
	   decode_rsm_set_els(__TopXMLNS, _els, After, Last, First,
			      Count, decode_rsm_before(__TopXMLNS, _el), Max,
			      Index);
       true ->
	   decode_rsm_set_els(__TopXMLNS, _els, After, Last, First,
			      Count, Before, Max, Index)
...
decode_rsm_set_els(__TopXMLNS,
    ...
    if _xmlns == <<>>; _xmlns == __TopXMLNS ->
	   decode_rsm_set_els(__TopXMLNS, _els, After, Last, First,
			      decode_rsm_count(__TopXMLNS, _el), Before, Max,
			      Index);
       true ->
	   decode_rsm_set_els(__TopXMLNS, _els, After, Last, First,
			      Count, Before, Max, Index)
...
decode_rsm_set_els(__TopXMLNS,
    ...
    _xmlns = get_attr(<<"xmlns">>, _attrs),
    if _xmlns == <<>>; _xmlns == __TopXMLNS ->
	   decode_rsm_set_els(__TopXMLNS, _els, After, Last,
			      decode_rsm_first(__TopXMLNS, _el), Count, Before,
			      Max, Index);
       true ->
	   decode_rsm_set_els(__TopXMLNS, _els, After, Last, First,
			      Count, Before, Max, Index)
...
decode_rsm_set_els(__TopXMLNS,
    ...
    if _xmlns == <<>>; _xmlns == __TopXMLNS ->
	   decode_rsm_set_els(__TopXMLNS, _els, After, Last, First,
			      Count, Before, Max,
			      decode_rsm_index(__TopXMLNS, _el));
       true ->
	   decode_rsm_set_els(__TopXMLNS, _els, After, Last, First,
			      Count, Before, Max, Index)
    end;
decode_rsm_set_els(__TopXMLNS,
    ...
    _xmlns = get_attr(<<"xmlns">>, _attrs),
    if _xmlns == <<>>; _xmlns == __TopXMLNS ->
	   decode_rsm_set_els(__TopXMLNS, _els, After,
			      decode_rsm_last(__TopXMLNS, _el), First, Count,
			      Before, Max, Index);
       true ->
	   decode_rsm_set_els(__TopXMLNS, _els, After, Last, First,
			      Count, Before, Max, Index)
...
decode_rsm_set_els(__TopXMLNS,
    ...
    if _xmlns == <<>>; _xmlns == __TopXMLNS ->
	   decode_rsm_set_els(__TopXMLNS, _els, After, Last, First,
			      Count, Before, decode_rsm_max(__TopXMLNS, _el),
			      Index);
       true ->
	   decode_rsm_set_els(__TopXMLNS, _els, After, Last, First,
			      Count, Before, Max, Index)
    end;
decode_rsm_set_els(__TopXMLNS, [_ | _els], After, Last,
...
decode_rsm_first(__TopXMLNS,
		 {xmlel, <<"first">>, _attrs, _els}) ->
    Data = decode_rsm_first_els(__TopXMLNS, _els, <<>>),
    Index = decode_rsm_first_attrs(__TopXMLNS, _attrs,
				   undefined),
    {rsm_first, Index, Data}.

decode_rsm_first_els(__TopXMLNS, [], Data) ->
    decode_rsm_first_cdata(__TopXMLNS, Data);
decode_rsm_first_els(__TopXMLNS,
		     [{xmlcdata, _data} | _els], Data) ->
    decode_rsm_first_els(__TopXMLNS, _els,
			 <<Data/binary, _data/binary>>);
decode_rsm_first_els(__TopXMLNS, [_ | _els], Data) ->
    decode_rsm_first_els(__TopXMLNS, _els, Data).
...
decode_rsm_max(__TopXMLNS,
	       {xmlel, <<"max">>, _attrs, _els}) ->
    Cdata = decode_rsm_max_els(__TopXMLNS, _els, <<>>),
    Cdata.

decode_rsm_max_els(__TopXMLNS, [], Cdata) ->
    decode_rsm_max_cdata(__TopXMLNS, Cdata);
decode_rsm_max_els(__TopXMLNS,
		   [{xmlcdata, _data} | _els], Cdata) ->
    decode_rsm_max_els(__TopXMLNS, _els,
		       <<Cdata/binary, _data/binary>>);
decode_rsm_max_els(__TopXMLNS, [_ | _els], Cdata) ->
    decode_rsm_max_els(__TopXMLNS, _els, Cdata).
...
decode_rsm_index(__TopXMLNS,
		 {xmlel, <<"index">>, _attrs, _els}) ->
    Cdata = decode_rsm_index_els(__TopXMLNS, _els, <<>>),
    Cdata.

decode_rsm_index_els(__TopXMLNS, [], Cdata) ->
    decode_rsm_index_cdata(__TopXMLNS, Cdata);
decode_rsm_index_els(__TopXMLNS,
		     [{xmlcdata, _data} | _els], Cdata) ->
    decode_rsm_index_els(__TopXMLNS, _els,
			 <<Cdata/binary, _data/binary>>);
decode_rsm_index_els(__TopXMLNS, [_ | _els], Cdata) ->
    decode_rsm_index_els(__TopXMLNS, _els, Cdata).
...
decode_rsm_count(__TopXMLNS,
		 {xmlel, <<"count">>, _attrs, _els}) ->
    Cdata = decode_rsm_count_els(__TopXMLNS, _els, <<>>),
    Cdata.

decode_rsm_count_els(__TopXMLNS, [], Cdata) ->
    decode_rsm_count_cdata(__TopXMLNS, Cdata);
decode_rsm_count_els(__TopXMLNS,
		     [{xmlcdata, _data} | _els], Cdata) ->
    decode_rsm_count_els(__TopXMLNS, _els,
			 <<Cdata/binary, _data/binary>>);
decode_rsm_count_els(__TopXMLNS, [_ | _els], Cdata) ->
    decode_rsm_count_els(__TopXMLNS, _els, Cdata).
...
decode_rsm_last(__TopXMLNS,
		{xmlel, <<"last">>, _attrs, _els}) ->
    Cdata = decode_rsm_last_els(__TopXMLNS, _els, <<>>),
    Cdata.

decode_rsm_last_els(__TopXMLNS, [], Cdata) ->
    decode_rsm_last_cdata(__TopXMLNS, Cdata);
decode_rsm_last_els(__TopXMLNS,
		    [{xmlcdata, _data} | _els], Cdata) ->
    decode_rsm_last_els(__TopXMLNS, _els,
			<<Cdata/binary, _data/binary>>);
decode_rsm_last_els(__TopXMLNS, [_ | _els], Cdata) ->
    decode_rsm_last_els(__TopXMLNS, _els, Cdata).
...
decode_rsm_before(__TopXMLNS,
		  {xmlel, <<"before">>, _attrs, _els}) ->
    Cdata = decode_rsm_before_els(__TopXMLNS, _els, <<>>),
    Cdata.

decode_rsm_before_els(__TopXMLNS, [], Cdata) ->
    decode_rsm_before_cdata(__TopXMLNS, Cdata);
decode_rsm_before_els(__TopXMLNS,
		      [{xmlcdata, _data} | _els], Cdata) ->
    decode_rsm_before_els(__TopXMLNS, _els,
			  <<Cdata/binary, _data/binary>>);
decode_rsm_before_els(__TopXMLNS, [_ | _els], Cdata) ->
    decode_rsm_before_els(__TopXMLNS, _els, Cdata).
...
decode_rsm_after(__TopXMLNS,
		 {xmlel, <<"after">>, _attrs, _els}) ->
    Cdata = decode_rsm_after_els(__TopXMLNS, _els, <<>>),
    Cdata.

decode_rsm_after_els(__TopXMLNS, [], Cdata) ->
    decode_rsm_after_cdata(__TopXMLNS, Cdata);
decode_rsm_after_els(__TopXMLNS,
		     [{xmlcdata, _data} | _els], Cdata) ->
    decode_rsm_after_els(__TopXMLNS, _els,
			 <<Cdata/binary, _data/binary>>);
decode_rsm_after_els(__TopXMLNS, [_ | _els], Cdata) ->
    decode_rsm_after_els(__TopXMLNS, _els, Cdata).
...
decode_p1_standby(__TopXMLNS,
		  {xmlel, <<"standby">>, _attrs, _els}) ->
    Set = decode_p1_standby_els(__TopXMLNS, _els, <<>>),
    {p1_standby, Set}.

decode_p1_standby_els(__TopXMLNS, [], Set) ->
    decode_p1_standby_cdata(__TopXMLNS, Set);
decode_p1_standby_els(__TopXMLNS,
		      [{xmlcdata, _data} | _els], Set) ->
    decode_p1_standby_els(__TopXMLNS, _els,
			  <<Set/binary, _data/binary>>);
decode_p1_standby_els(__TopXMLNS, [_ | _els], Set) ->
    decode_p1_standby_els(__TopXMLNS, _els, Set).
...
decode_p1_standby_cdata(__TopXMLNS, <<>>) ->
    erlang:error({xmpp_codec,
		  {missing_cdata, <<>>, <<"standby">>, __TopXMLNS}});
decode_p1_standby_cdata(__TopXMLNS, _val) ->
    case catch dec_bool(_val) of
      {'EXIT', _} ->
	  erlang:error({xmpp_codec,
			{bad_cdata_value, <<>>, <<"standby">>, __TopXMLNS}});
      _res -> _res
    end.
...
decode_p1_rebind(__TopXMLNS,
    ...
    {Sid, Jid} = decode_p1_rebind_els(__TopXMLNS, _els,
				      undefined, undefined),
    {p1_rebind, Jid, Sid}.

decode_p1_rebind_els(__TopXMLNS, [], Sid, Jid) ->
    {Sid, Jid};
decode_p1_rebind_els(__TopXMLNS,
		     [{xmlel, <<"jid">>, _attrs, _} = _el | _els], Sid,
		     Jid) ->
    _xmlns = get_attr(<<"xmlns">>, _attrs),
    if _xmlns == <<>>; _xmlns == __TopXMLNS ->
	   decode_p1_rebind_els(__TopXMLNS, _els, Sid,
				decode_p1_rebind_jid(__TopXMLNS, _el));
       true -> decode_p1_rebind_els(__TopXMLNS, _els, Sid, Jid)
    end;
decode_p1_rebind_els(__TopXMLNS,
		     [{xmlel, <<"sid">>, _attrs, _} = _el | _els], Sid,
		     Jid) ->
    _xmlns = get_attr(<<"xmlns">>, _attrs),
    if _xmlns == <<>>; _xmlns == __TopXMLNS ->
	   decode_p1_rebind_els(__TopXMLNS, _els,
				decode_p1_rebind_sid(__TopXMLNS, _el), Jid);
       true -> decode_p1_rebind_els(__TopXMLNS, _els, Sid, Jid)
    end;
decode_p1_rebind_els(__TopXMLNS, [_ | _els], Sid,
		     Jid) ->
    decode_p1_rebind_els(__TopXMLNS, _els, Sid, Jid).
...
decode_p1_rebind_sid(__TopXMLNS,
		     {xmlel, <<"sid">>, _attrs, _els}) ->
    Cdata = decode_p1_rebind_sid_els(__TopXMLNS, _els,
				     <<>>),
    Cdata.

decode_p1_rebind_sid_els(__TopXMLNS, [], Cdata) ->
    decode_p1_rebind_sid_cdata(__TopXMLNS, Cdata);
decode_p1_rebind_sid_els(__TopXMLNS,
			 [{xmlcdata, _data} | _els], Cdata) ->
    decode_p1_rebind_sid_els(__TopXMLNS, _els,
			     <<Cdata/binary, _data/binary>>);
decode_p1_rebind_sid_els(__TopXMLNS, [_ | _els],
			 Cdata) ->
    decode_p1_rebind_sid_els(__TopXMLNS, _els, Cdata).
...
decode_p1_rebind_sid_cdata(__TopXMLNS, <<>>) ->
    erlang:error({xmpp_codec,
		  {missing_cdata, <<>>, <<"sid">>, __TopXMLNS}});
decode_p1_rebind_sid_cdata(__TopXMLNS, _val) -> _val.

encode_p1_rebind_sid_cdata(_val, _acc) ->
    [{xmlcdata, _val} | _acc].

decode_p1_rebind_jid(__TopXMLNS,
		     {xmlel, <<"jid">>, _attrs, _els}) ->
    Cdata = decode_p1_rebind_jid_els(__TopXMLNS, _els,
				     <<>>),
    Cdata.

decode_p1_rebind_jid_els(__TopXMLNS, [], Cdata) ->
    decode_p1_rebind_jid_cdata(__TopXMLNS, Cdata);
decode_p1_rebind_jid_els(__TopXMLNS,
			 [{xmlcdata, _data} | _els], Cdata) ->
    decode_p1_rebind_jid_els(__TopXMLNS, _els,
			     <<Cdata/binary, _data/binary>>);
decode_p1_rebind_jid_els(__TopXMLNS, [_ | _els],
			 Cdata) ->
    decode_p1_rebind_jid_els(__TopXMLNS, _els, Cdata).
...
decode_p1_rebind_jid_cdata(__TopXMLNS, <<>>) ->
    erlang:error({xmpp_codec,
		  {missing_cdata, <<>>, <<"jid">>, __TopXMLNS}});
decode_p1_rebind_jid_cdata(__TopXMLNS, _val) ->
    case catch dec_jid(_val) of
      {'EXIT', _} ->
	  erlang:error({xmpp_codec,
			{bad_cdata_value, <<>>, <<"jid">>, __TopXMLNS}});
...
decode_p1_rebind_failure(__TopXMLNS,
			 {xmlel, <<"failure">>, _attrs, _els}) ->
    Reason = decode_p1_rebind_failure_els(__TopXMLNS, _els,
					  <<>>),
    {p1_rebind_failure, Reason}.

decode_p1_rebind_failure_els(__TopXMLNS, [], Reason) ->
    decode_p1_rebind_failure_cdata(__TopXMLNS, Reason);
decode_p1_rebind_failure_els(__TopXMLNS,
			     [{xmlcdata, _data} | _els], Reason) ->
    decode_p1_rebind_failure_els(__TopXMLNS, _els,
				 <<Reason/binary, _data/binary>>);
decode_p1_rebind_failure_els(__TopXMLNS, [_ | _els],
			     Reason) ->
    decode_p1_rebind_failure_els(__TopXMLNS, _els, Reason).
...
decode_p1_rebind_failure_cdata(__TopXMLNS, <<>>) ->
    undefined;
decode_p1_rebind_failure_cdata(__TopXMLNS, _val) ->
    _val.
...
decode_p1_push(__TopXMLNS,
	...
	decode_p1_push_els(__TopXMLNS, _els, undefined,
			   undefined, undefined),
    {p1_push, Keepalive, Session, Status}.

decode_p1_push_els(__TopXMLNS, [], Status, Keepalive,
		   Session) ->
    {Status, Keepalive, Session};
decode_p1_push_els(__TopXMLNS,
		   [{xmlel, <<"keepalive">>, _attrs, _} = _el | _els],
		   Status, Keepalive, Session) ->
    _xmlns = get_attr(<<"xmlns">>, _attrs),
    if _xmlns == <<>>; _xmlns == __TopXMLNS ->
	   decode_p1_push_els(__TopXMLNS, _els, Status,
			      decode_p1_push_keepalive(__TopXMLNS, _el),
			      Session);
       true ->
	   decode_p1_push_els(__TopXMLNS, _els, Status, Keepalive,
			      Session)
    end;
decode_p1_push_els(__TopXMLNS,
		   [{xmlel, <<"session">>, _attrs, _} = _el | _els],
		   Status, Keepalive, Session) ->
    _xmlns = get_attr(<<"xmlns">>, _attrs),
    if _xmlns == <<>>; _xmlns == __TopXMLNS ->
	   decode_p1_push_els(__TopXMLNS, _els, Status, Keepalive,
			      decode_p1_push_session(__TopXMLNS, _el));
       true ->
	   decode_p1_push_els(__TopXMLNS, _els, Status, Keepalive,
			      Session)
    end;
decode_p1_push_els(__TopXMLNS,
    ...
    _xmlns = get_attr(<<"xmlns">>, _attrs),
    if _xmlns == <<>>; _xmlns == __TopXMLNS ->
	   decode_p1_push_els(__TopXMLNS, _els,
			      decode_p1_push_status(__TopXMLNS, _el), Keepalive,
			      Session);
       true ->
	   decode_p1_push_els(__TopXMLNS, _els, Status, Keepalive,
			      Session)
    end;
decode_p1_push_els(__TopXMLNS, [_ | _els], Status,
		   Keepalive, Session) ->
    decode_p1_push_els(__TopXMLNS, _els, Status, Keepalive,
		       Session).
...
decode_p1_push_status(__TopXMLNS,
		      {xmlel, <<"status">>, _attrs, _els}) ->
    Text = decode_p1_push_status_els(__TopXMLNS, _els,
				     <<>>),
    Type = decode_p1_push_status_attrs(__TopXMLNS, _attrs,
				       undefined),
    {p1_push_status, Type, Text}.

decode_p1_push_status_els(__TopXMLNS, [], Text) ->
    decode_p1_push_status_cdata(__TopXMLNS, Text);
decode_p1_push_status_els(__TopXMLNS,
			  [{xmlcdata, _data} | _els], Text) ->
    decode_p1_push_status_els(__TopXMLNS, _els,
			      <<Text/binary, _data/binary>>);
decode_p1_push_status_els(__TopXMLNS, [_ | _els],
			  Text) ->
    decode_p1_push_status_els(__TopXMLNS, _els, Text).

decode_p1_push_status_attrs(__TopXMLNS,
			    [{<<"type">>, _val} | _attrs], _Type) ->
    decode_p1_push_status_attrs(__TopXMLNS, _attrs, _val);
decode_p1_push_status_attrs(__TopXMLNS, [_ | _attrs],
			    Type) ->
    decode_p1_push_status_attrs(__TopXMLNS, _attrs, Type);
decode_p1_push_status_attrs(__TopXMLNS, [], Type) ->
    decode_p1_push_status_attr_type(__TopXMLNS, Type).
...
decode_p1_push_status_attr_type(__TopXMLNS,
				undefined) ->
    undefined;
decode_p1_push_status_attr_type(__TopXMLNS, _val) ->
...
decode_p1_push_status_cdata(__TopXMLNS, <<>>) ->
    undefined;
decode_p1_push_status_cdata(__TopXMLNS, _val) -> _val.

encode_p1_push_status_cdata(undefined, _acc) -> _acc;
encode_p1_push_status_cdata(_val, _acc) ->
    [{xmlcdata, _val} | _acc].

decode_p1_push_session(__TopXMLNS,
		       {xmlel, <<"session">>, _attrs, _els}) ->
    Duration = decode_p1_push_session_attrs(__TopXMLNS,
					    _attrs, undefined),
    Duration.

decode_p1_push_session_attrs(__TopXMLNS,
			     [{<<"duration">>, _val} | _attrs], _Duration) ->
    decode_p1_push_session_attrs(__TopXMLNS, _attrs, _val);
decode_p1_push_session_attrs(__TopXMLNS, [_ | _attrs],
			     Duration) ->
    decode_p1_push_session_attrs(__TopXMLNS, _attrs,
				 Duration);
decode_p1_push_session_attrs(__TopXMLNS, [],
			     Duration) ->
    decode_p1_push_session_attr_duration(__TopXMLNS,
					 Duration).
...
decode_p1_push_keepalive(__TopXMLNS,
			 {xmlel, <<"keepalive">>, _attrs, _els}) ->
    Max = decode_p1_push_keepalive_attrs(__TopXMLNS, _attrs,
					 undefined),
    Max.

decode_p1_push_keepalive_attrs(__TopXMLNS,
			       [{<<"max">>, _val} | _attrs], _Max) ->
    decode_p1_push_keepalive_attrs(__TopXMLNS, _attrs,
				   _val);
decode_p1_push_keepalive_attrs(__TopXMLNS, [_ | _attrs],
			       Max) ->
    decode_p1_push_keepalive_attrs(__TopXMLNS, _attrs, Max);
decode_p1_push_keepalive_attrs(__TopXMLNS, [], Max) ->
    decode_p1_push_keepalive_attr_max(__TopXMLNS, Max).
...
decode_p1_push_keepalive_attr_max(__TopXMLNS,
				  undefined) ->
    erlang:error({xmpp_codec,
		  {missing_attr, <<"max">>, <<"keepalive">>,
		   __TopXMLNS}});
decode_p1_push_keepalive_attr_max(__TopXMLNS, _val) ->
    case catch dec_int(_val, 0, infinity) of
      {'EXIT', _} ->
	  erlang:error({xmpp_codec,
